...
##################################################################################################
DAYS_TO_COMPARE=20 ## (5 is 1 trading week (5 days per week) )
...
while read Blue_Chip; do
i=${Blue_Chip}.STOCK
#echo "############################# ANALYZING ${i} ##########################################"

alert() {
  echo "$ALERT_MESSAGE" | mail -s "BLUE-CHIP: PRICE-CHANGE in `date "+(%m-%d-%Y_%H:%M)"`" demoo.nooob@gmail.com
...
done < $STOCK_HOME/input/BlueChipHolding.txt
